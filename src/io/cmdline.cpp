...
int g_pending_audio_delay_ms = 0; // Audio delay pending application after LDP init
    ...
    if (!bHomeDirSet) {
        ...
        if (homeenv == NULL) {
            ...
            if (internal != NULL && internal[0] != 0) {
                g_homedir.set_homedir(internal);
            }
        }
#endif
...
bool parse_cmd_line(int argc, char **argv)
{
    ...
    if (parse_subsystems() && parse_romdir() && parse_ramdir() && parse_homedir() &&
        ...
        for (;;) {
            ...
            // if they are setting an audio delay to fix Singe 2 sync issues
            // (Singe 2 audio bug work-around: audio may be out of sync)
            else if (strcasecmp(s, "-audio-delay") == 0) {
                get_next_word(s, sizeof(s));
                int delay_ms = static_cast<int>(atoi(s));
                if (delay_ms < 0) delay_ms = 0;

                // Apply immediately to VLDP global so this works even if
                // command-line parsing occurs before g_ldp is fully initialized.
                extern int g_audio_delay_ms;
                g_audio_delay_ms = delay_ms;
                g_pending_audio_delay_ms = delay_ms;

                char msg[256];
                snprintf(msg, sizeof(msg), "Setting audio delay to %d ms", delay_ms);
                printline(msg);
            }
