...
static int get_controller_slot(SDL_JoystickID instance_id)
{
...
{
    const int which = get_controller_slot(event->cbutton.which);
    if (which < 0) {
        return false;
    }
...
// cast would fail if g_game is not a thayers class
    ...
    case SDL_CONTROLLERBUTTONDOWN:
        ...
        if (g_game->ControllerButtonProxy(event->cbutton.button, true, which)) {
            break;
        }
    ...
    case SDL_CONTROLLERBUTTONUP:
        ...
        if (g_game->ControllerButtonProxy(event->cbutton.button, false, which)) {
            break;
        }
        if (g_mouse_mode == MANY_MOUSE)
    ...
    case SDL_JOYAXISMOTION:
        if (g_use_gamepad && g_gamepad_attached > 0) break;
...
void process_controller_motion(SDL_Event *event)
{
    ...
    const int which = get_controller_slot(event->caxis.which);
    if (which < 0) {
        return;
    }
