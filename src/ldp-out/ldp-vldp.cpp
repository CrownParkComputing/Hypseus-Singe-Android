...
bool ldp_vldp::init_player()
{
    ...
    // Apply any pending audio delay that was captured from command line
    // (deferred until LDP initialization completes)
    extern int g_pending_audio_delay_ms;
    if (result && g_pending_audio_delay_ms > 0) {
        set_audio_delay(g_pending_audio_delay_ms);
        LOGI << fmt("Applied pending audio delay: %d ms", g_pending_audio_delay_ms);
    }
...
bool ldp_vldp::nudge_audio_delay(int delta_ms)
{
    extern int g_audio_delay_ms;

    int new_delay_ms = g_audio_delay_ms + delta_ms;
    if (new_delay_ms < 0) {
        new_delay_ms = 0;
    }

    g_audio_delay_ms = new_delay_ms;
    LOGI << fmt("Runtime audio delay set to %d ms", g_audio_delay_ms);
    ...
    if (m_audio_file_opened) {
        ...
        LOGW << "Runtime audio delay reseek failed";
        return false;
    }
...
int ldp_vldp::get_audio_delay_ms() const
{
    extern int g_audio_delay_ms;
    return g_audio_delay_ms;
}
...
void ldp_vldp::think()
{
    ...
    // Periodic audio/video sync correction to prevent drift
    // Every ~15 frames @ 30fps (~500ms), recalculate elapsed time from frame position
    static Uint32 s_sync_counter = 0;

    if (m_status == LDP_PLAYING) {
        if (s_sync_counter++ >= 15) {  // Resync every ~500ms
            s_sync_counter = 0;

            // Recalculate expected elapsed time based on current video frame position
            uint32_t current_frame = 0;
            if (g_vldp_info && g_vldp_info->current_frame >= m_target_mpegframe) {
                current_frame = g_vldp_info->current_frame - m_target_mpegframe;
            }

            // Calculate what the elapsed time SHOULD be based on frame position
            // Using frame count / FPS to determine expected elapsed milliseconds
            Uint32 fpks = g_game ? g_game->get_disc_fpks() : 30000;  // frames per 1000 seconds
            Uint32 expected_elapsed_ms = (current_frame * 1000000UL) / fpks;

            // If there's drift > 50ms, correct it gradually
            Sint32 drift = (Sint32)m_uElapsedMsSincePlay - (Sint32)expected_elapsed_ms;
            if (drift > 50 || drift < -50) {
                LOGD << fmt("Audio/video drift: actual=%u expected=%u drift=%d ms, resyncing",
                            m_uElapsedMsSincePlay, expected_elapsed_ms, drift);
                // Gradual correction: move 25% of the way toward expected time
                m_uElapsedMsSincePlay = m_uElapsedMsSincePlay + (drift / 4);
            }
        }
    } else {
        s_sync_counter = 0;
...
Uint64 ldp_vldp::get_audio_sample_position(uint32_t uTargetMpegFrame)
{
    ...
    Uint64 u64BaseAudioTargetPos = 0;
    ...
    u64BaseAudioTargetPos = u64AudioTargetPos;

    // Apply user-configured audio delay to every audio seek position.
    // This keeps the offset stable across search/skip/speed-change paths.
    if (g_audio_delay_ms > 0) {
        const Uint64 u64AudioDelaySamples = (static_cast<Uint64>(g_audio_delay_ms) * sound::FREQ) / 1000;
        u64AudioTargetPos += u64AudioDelaySamples;
    }

    static unsigned int s_audio_seek_log_count = 0;
    if (s_audio_seek_log_count < 12) {
        ++s_audio_seek_log_count;
        LOGI << fmt("Audio target: frame=%u base=%llu delay_ms=%d final=%llu",
                    uTargetMpegFrame,
                    (unsigned long long)u64BaseAudioTargetPos,
                    g_audio_delay_ms,
                    (unsigned long long)u64AudioTargetPos);
    }
