...
string m_oggpath         = "";
int g_audio_delay_ms     = 0; // audio delay in milliseconds (for Singe 2 sync fix)
        ...
        if (g_big_buf) {
            ...
            if (open_result == 0) {
                ...
                if ((info->channels == 2) && (info->rate == 44100)) {
                    g_audio_ready = true;
                    result        = true;
                    LOGI << fmt("Audio stream open: delay_ms=%d", g_audio_delay_ms);
                    
                    // Apply audio delay if specified (Singe 2 audio sync workaround)
                    if (g_audio_delay_ms > 0) {
                        // Convert milliseconds to samples (44100 Hz = 44.1 samples per ms)
                        Uint64 delay_samples = (Uint64)((g_audio_delay_ms / 1000.0) * 44100);
                        // Seek forward to apply the delay offset
                        if (seek_audio(delay_samples)) {
                            LOGD << fmt("Applied audio delay: %d ms (%llu samples)", 
                                        g_audio_delay_ms, (unsigned long long)delay_samples);
                        } else {
                            LOGW << "Failed to seek audio for delay";
                        }
                    }
                } else {
