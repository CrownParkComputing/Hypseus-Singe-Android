...
// Audio delay display timer (for 1-second timeout)
static Uint32 g_audio_delay_display_timeout_ms = 0;
...
static SDL_Renderer *create_renderer_with_fallback(SDL_Window *window, Uint32 flags)
{
    SDL_Renderer *renderer = nullptr;

    const char *driver_chain[5] = {nullptr, nullptr, nullptr, nullptr, nullptr};
    int chain_len = 0;

    if (g_vulkan) {
        driver_chain[chain_len++] = "vulkan";
        driver_chain[chain_len++] = "opengles2";
        driver_chain[chain_len++] = "opengl";
        driver_chain[chain_len++] = "software";
    } else if (g_opengl) {
        driver_chain[chain_len++] = "opengles2";
        driver_chain[chain_len++] = "opengl";
        driver_chain[chain_len++] = "software";
    ...
    for (int i = 0; i < chain_len; ++i) {
        SDL_SetHint(SDL_HINT_RENDER_DRIVER, driver_chain[i]);
        renderer = SDL_CreateRenderer(window, -1, flags);
        if (renderer) {
            LOGI << fmt("Using SDL renderer backend: %s", driver_chain[i]);
            return renderer;
        }
    }
...
bool init_display()
{
    ...
    // If already fully initialized, return success immediately.
    // This allows safe re-entry from game::init_video() after hypseus.cpp
    // has already called init_display() to satisfy SDL_input_init() ordering.
    if (g_window && g_renderer && !g_teardown) {
        LOGI << "init_display: already initialized, skipping";
        return true;
    }
    ...
    Uint32 sdl_render_flags = SDL_RENDERER_TARGETTEXTURE;
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) >= 0) {
        ...
        if (!g_window) {
            ...
            Uint32 sdl_sb_render_flags = sdl_render_flags;

            if (g_vsync && (sdl_render_flags & SDL_RENDERER_ACCELERATED))
                sdl_render_flags |= SDL_RENDERER_PRESENTVSYNC;

            g_renderer = create_renderer_with_fallback(g_window, sdl_render_flags);
            ...
            if (!g_renderer) {
                ...
                if (g_keyboard_bezel) {
                    ...
                    if (!mpo_file_exists(g_tqkeys.c_str()))
                        g_tqkeys = g_homedir.find_file("pics/tqkeys.png", true);
                }
                ...
                if (g_game->m_software_scoreboard) {
                    ...
                    if (g_sb_bezel) {
                        ...
                        g_sb_renderer = create_renderer_with_fallback(g_sb_window, sdl_sb_render_flags);
                        ...
                        LOGI << "Android build: skipping secondary scoreboard window.";
                    }
#endif
                    else if (!notify) {
...
static std::string resolve_pic_path(const char *filename)
{
    return g_homedir.find_file(fmt("pics/%s", filename), true);
}

static SDL_Surface *load_led_strip(const char *filename)
{
    std::string filepath = resolve_pic_path(filename);
...
static SDL_Surface *load_one_bmp(const char *filename, bool bezel)
{
    ...
    if (!mpo_file_exists(filepath.c_str()))
        filepath = resolve_pic_path(filename);
...
void vid_draw_audio_delay_display()
{
    // Display audio delay on the overlay in the top-right corner
    // Only show for 1 second after last L1/L2 press
    
    if (!g_overlay_blitter || !g_ldp) {
        return;
    }

    ldp_vldp *cur_ldp = dynamic_cast<ldp_vldp *>(g_ldp);
    if (!cur_ldp) {
        return;
    }

    // Check if display should be shown (within 1 second of last update)
    Uint32 current_ticks = GET_TICKS();
    if (g_audio_delay_display_timeout_ms == 0 || (current_ticks - g_audio_delay_display_timeout_ms) > 1000) {
        // Timeout reached or not activated, don't display
        return;
    }

    int delay_ms = cur_ldp->get_audio_delay_ms();
    char delay_text[32];
    snprintf(delay_text, sizeof(delay_text), "Delay: %d ms", delay_ms);

    // Position: top-right of the overlay
    int col = g_overlay_width / 6 - strlen(delay_text);  // right align
    int row = 0;  // top

    // Only draw if we have valid dimensions
    if (col > 0 && row >= 0) {
        draw_string(delay_text, col, row, g_overlay_blitter);
        g_overlay_needs_update = true;
    }
}

// Call this when audio delay is adjusted (L1/L2 press)
void vid_activate_delay_display()
{
    g_audio_delay_display_timeout_ms = GET_TICKS();
}
...
void vid_blit()
{
    ...
    // Draw audio delay display on the overlay
    vid_draw_audio_delay_display();
