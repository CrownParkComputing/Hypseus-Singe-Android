...
int hypseus_run(int argc, char **argv, bool set_current_dir)
{
#ifdef __ANDROID__
    static plog::AndroidAppender<plog::TxtFormatter> androidAppender("HypseusNative");
    if (!plog::get()) {
        plog::init(LOGLEVEL, &androidAppender);
    }
#endif
    ...
    if (set_current_dir && argc > 0 && argv && argv[0])
    ...
    if (parse_cmd_line(argc, argv)) {
        ...
        LOGI << "DEBUG: Calling load_bmps then init_display";
	if (video::load_bmps() && video::init_display()) {
            LOGI << "DEBUG: load_bmps+init_display OK, calling sound::init";
            if (sound::init()) {
                LOGI << "DEBUG: sound::init OK, calling SDL_input_init";
                if (SDL_input_init()) {
                    LOGI << "DEBUG: SDL_input_init OK, calling load_roms";
                    // if the roms were loaded successfully
                    if (g_game->load_roms()) {
                        LOGI << "DEBUG: load_roms OK, calling init_video";
                        // if the video was initialized successfully
                        LOGI << "DEBUG: About to call g_game->init_video";
                        if (g_game->init_video()) {
                            LOGI << "DEBUG: g_game->init_video returned true";
                            ...
                            LOGI << "DEBUG: About to call g_ldp->pre_init";
                            if (g_ldp->pre_init()) {
                                LOGI << "DEBUG: g_ldp->pre_init returned true, calling g_game->pre_init";
                                if (g_game->pre_init()) // initialize all cpu's
                                {
                                    ...
                                    LOGI << "DEBUG: g_game->pre_init returned false";
                                }
                                g_ldp->pre_shutdown();
                            } else {
                                LOGI << "DEBUG: g_ldp->pre_init returned false";
                                if (!quitflag) {
                                    printerror(
...
// explicitly here with extern "C" linkage so dlsym() can find it.
...
#ifdef __ANDROID__
    static plog::AndroidAppender<plog::TxtFormatter> androidAppender("HypseusNative");
    plog::init(LOGLEVEL, &androidAppender).addAppender(&consoleAppender).addAppender(&fileAppender);
#else
