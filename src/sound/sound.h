...
// Lock/unlock the SDL audio device that was opened via SDL_OpenAudioDevice.
// samples.cpp must use these instead of SDL_LockAudio() / SDL_UnlockAudio()
// (which lock the legacy compat device 1, not g_audio_device).
void lock_audio();
void unlock_audio();
}
