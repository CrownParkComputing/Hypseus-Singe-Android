...
void shutdown()
{
    ...
    if (g_sound_initialized) {
        ...
        g_soundmix_callback = mixNone; // reset mixer before closing device
        SDL_CloseAudioDevice(g_audio_device);
        g_audio_device = 0;            // reset so SDL_LockAudioDevice(0) is a no-op
...
void shutdown_chip()
{
    ...
    g_chip_head = NULL;        // reset so re-init works if activity restarts
    g_uSoundChipNextID = 0;    // reset chip ID counter
    UNLOCK_AUDIO(g_audio_device);
}
