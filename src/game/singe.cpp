...
static bool handle_runtime_audio_delay_key(SDL_Keycode key)
{
    if (key != SDLK_F7 && key != SDLK_F8) {
        return false;
    }

    ldp_vldp *cur_ldp = dynamic_cast<ldp_vldp *>(g_ldp);
    if (!cur_ldp) {
        printline("Runtime audio delay is only available with VLDP.");
        return true;
    }

    const int delta_ms = (key == SDLK_F8) ? 50 : -50;
    cur_ldp->nudge_audio_delay(delta_ms);

    char msg[128];
    snprintf(msg,
             sizeof(msg),
             "Runtime audio delay: %d ms (F7 -50ms, F8 +50ms)",
             cur_ldp->get_audio_delay_ms());
    printline(msg);
...
{
    // Audio delay runtime adjustment with L1/L2
    if (input == SWITCH_SKILL1 || input == SWITCH_SKILL2) {
        ldp_vldp *cur_ldp = dynamic_cast<ldp_vldp *>(g_ldp);
        if (cur_ldp) {
            ...
            snprintf(msg,
                     sizeof(msg),
                     "Audio delay: %d ms (L1 -50ms, L2 +50ms)",
                     cur_ldp->get_audio_delay_ms());
            printline(msg);
        }
        return;
...
void singe::process_keydown(SDL_Keycode key, int keydefs[][2])
{
    ...
    { // traverse the keydef array for mapped keys.
        ...
        // Keep core control mappings active even in full keyboard mode so
        // remapped Android/controller keys still hit SWITCH_* actions.
        for (Uint8 move = 0; move < SWITCH_COUNT; move++) {
            if ((key == keydefs[move][0]) || (key == keydefs[move][1])) {
                if (move != SWITCH_PAUSE) input_enable(move, NOMOUSE);
                return;
...
void singe::process_keyup(SDL_Keycode key, int keydefs[][2])
{
    ...
    // Live sync tuning hotkeys for Singe titles.
    if (handle_runtime_audio_delay_key(key)) {
        return;
    }
    if (i_keyboard_mode == KEYBD_NORMAL) // Using normal keyboard mappings
    { // traverse the keydef array for mapped keys.
        ...
        for (Uint8 move = 0; move < SWITCH_COUNT; move++) {
            if ((key == keydefs[move][0]) || (key == keydefs[move][1])) {
                // Match normal mode behavior for mapped quit/pause keys.
                if (move == SWITCH_PAUSE) {
                    toggle_game_pause();
                    input_disable(SWITCH_PAUSE, NOMOUSE);
                } else if (move == i_keyboard_quit) {
                    if (m_running) set_quitflag();
...
{
    // L2 live sync nudge: +50ms on press edge.
    if (axis == SDL_CONTROLLER_AXIS_TRIGGERLEFT) {
        static bool s_l2_pressed[MAX_GAMECONTROLLER] = { false };
        const bool pressed_now = (value > 16000);

        if (pressed_now && !s_l2_pressed[id]) {
            ldp_vldp *cur_ldp = dynamic_cast<ldp_vldp *>(g_ldp);
            if (cur_ldp) {
                cur_ldp->nudge_audio_delay(50);
                char msg[128];
                snprintf(msg,
                         sizeof(msg),
                         "Runtime audio delay: %d ms (L1 -50ms, L2 +50ms)",
                         cur_ldp->get_audio_delay_ms());
                printline(msg);
            }
            s_l2_pressed[id] = true;
        } else if (!pressed_now && s_l2_pressed[id]) {
            s_l2_pressed[id] = false;
        }
    }
...
bool singe::ControllerButtonProxy(Uint8 button, bool pressed, Uint8 id)
{
    // L1 live sync nudge: -50ms on button press.
    if (!pressed || button != SDL_CONTROLLER_BUTTON_LEFTSHOULDER) {
        return false;
    }
    ...
    // Consume this button so it does not trigger gameplay actions.
    return true;
}
