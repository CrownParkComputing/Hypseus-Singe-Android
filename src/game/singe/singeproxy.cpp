...
static SDL_Color             g_colorForeground     = {255, 255, 255, 255};
...
static bool controller_button_keyboard_fallback(int button)
{
    switch (button) {
    ...
    case SDL_CONTROLLER_BUTTON_Y:
        return g_keyboard_state[SDL_SCANCODE_SPACE] || g_keyboard_state[SDL_SCANCODE_LCTRL];
    case SDL_CONTROLLER_BUTTON_START:
        return g_keyboard_state[SDL_SCANCODE_1];
    case SDL_CONTROLLER_BUTTON_BACK:
        return g_keyboard_state[SDL_SCANCODE_5];
    default:
        return false;
    }
}
          ...
          if (lua_isnumber(L, 2))
              if (lua_isnumber(L, 3))
              {
                  ...
                  if (n == 4 && lua_isnumber(L, 4) ) {
                      ...
                      g_colorForeground.a = (uint8_t)255;
                  }
              }
  return 0;
}
...
static int sep_controller_button(lua_State *L)
{
    ...
    if (n > 0 && n <= 3) {
        if (lua_isnumber(L, 1)) {
            ...
            if (!d) {
                d = controller_button_keyboard_fallback(b);
                result = true;
            }
        }
    }
