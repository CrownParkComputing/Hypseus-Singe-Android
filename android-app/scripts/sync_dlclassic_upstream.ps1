param(
    ...
    [string]$RuntimeRoot = "../app/src/hypseus/assets/runtime/templates/dlclassic",
    [string]$ManifestPath = "../upstream/dlclassic-sync-lock.json",
    [switch]$KeepTemp
)
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("hypseus-dlclassic-sync-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $classicRootSrc = Join-Path $cloneDir "00-singe2\\dragons_lair_classic"
    if (-not (Test-Path -LiteralPath $classicRootSrc)) {
        throw "Directory 00-singe2/dragons_lair_classic not found."
    }

    Write-Host "[sync] Copying dragons_lair_classic template tree from: $classicRootSrc"
    ...
    Copy-Item -Path (Join-Path $classicRootSrc "*") -Destination $runtimeRootAbs -Recurse -Force
    ...
    $manifest = [PSCustomObject]@{
        ...
        outputs = [PSCustomObject]@{
            ...
            classicScriptTemplate = "dragons_lair_classic.singe"
            classicFramefileTemplate = "dragons_lair_classic.txt"
        }
